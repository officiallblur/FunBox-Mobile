...
async function parseJson(request: Request): Promise<ScrapeMoviePayload> {
  try {
    return (await request.json()) as ScrapeMoviePayload;
  } catch {
    return {};
  }
}

async function callResolver(title: string) {
  if (!RESOLVER_URL) {
  ...
  let json: { links?: Array<{ url: string; title?: string }> } = {};
  try {
...
Deno.serve(async (request) => {
  ...
  const movieTitle = payload.movieTitle?.trim() ?? '';
  if (!movieId || !movieTitle) {
    return jsonResponse({ success: false, message: 'movieId and movieTitle are required.' }, 400);
  }

  const resolved = await callResolver(movieTitle);
  ...
  const inserts = resolved.links
    ...
    .map((link) => ({
      movie_id: movieId,
      title: link.title?.trim() || movieTitle,
      url: link.url,
    }));

  if (inserts.length > 0) {
    const { error: insertError } = await adminClient.from('download_links').insert(inserts as any);
    if (insertError) {
